...
""",
    ...
    parser_protein.add_argument("pdb_id", help="PDB identifier (e.g., 8F7W)")
    parser_protein.add_argument("--output", "-o", help="Output PNG filename")
...
def run_2d(args: argparse.Namespace) -> None:
    try:
        ...
        if not args.auto_orient:
            overrides["angle_degrees"] = args.angle
        config = ConfigLoader.get_2d_config(overrides=overrides)
        gen = MoleculeGenerator2D(identifier=args.compound, config=config)
...
def run_3d(args: argparse.Namespace) -> None:
    try:
        ...
        if args.ray_shadows:
            overrides["ray_shadows"] = 1
        if args.depth_cue:
            overrides["depth_cue"] = 1
        config = ConfigLoader.get_3d_config(overrides=overrides)
        gen = MoleculeGenerator3D(identifier=args.compound, config=config)
